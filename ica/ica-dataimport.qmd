...
- Source: Generated from The Movie Database API.
- Creator: Kaggle. 
...
```{r}
library(tidyverse)
imdb_messy <- read_csv("C:/Users/bunny/Documents/STAT112/portfolio-manelson024/data/imdb_5000_messy.csv")
```
...
```{r}
imdb_temp <- readr::read_csv("../data/imdb_5000_messy.csv")
```
...
- There's a lot of NA values. 
- It's strange that the color variable has COLOR and color. 
- There's a movie that is 8 and a half hours long which is way too long for a movie. 
...
-   What type of info is provided on quantitative variables?
  - Minimum, 1st quadrant, median, 3rd quadrant, and maximum values. 
-   What type of info is provided on categorical variables?
  - The different properties of the variable and their counts. 
-   What stands out to you in these summaries? Is there anything you'd need to clean before using this data?
  - We definitely need to get rid of the NA values and fix the color variable. 
...
- That way, other people can follow our work and reproduce the data cleaning we do. 
...
imdb_messy |> 
  summarize(num_missing = sum(is.na(actor_1_facebook_likes)))
```
...
```
- It seems like films with a low number of voted users have a lot of NA values. 
...
- This removes the NAs from the calculation. 
...
-   In what situations might this function be a good idea?
  - It's good when we need complete info for each variable. 
-   In what situations might this function be a bad idea?
  - It's bad when we still need averages for some variables when some observations are only missing a couple variables we're not concerned about. 
