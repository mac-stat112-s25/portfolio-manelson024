...
```{r}
# Shows the month as either a number of an abbreviation. 
month(today)
month(today, label = TRUE)
```

```{r}
# This shows the week of the year. 
week(today)
```

```{r}
# This shows the day of the month (1-31)
mday(today)
yday(today)  # This is often called the "Julian day" and shows the day of the year (1-365)
```

```{r}
# This shwos the day of the week as either a number or an abbreviation. 
wday(today)
...
# Today is on or after February 14th, 2024. 
...
```{r}
# How many days of data do we have for each state? 240. 
Birthdays |>
  group_by(state) |>
  summarize(count(day))
...
Birthdays |>
  group_by(state) |>
  summarize(total_birth = count(births)) |>
  arrange(total_birth)
...
daily_births <- Birthdays |>
  group_by(day, wday) |>
  summarize(total_births_daily = sum(count(births)), .groups = "drop") 
```

Using this data, construct a plot of `births` over time, indicating the day of week.

```{r}
ggplot(daily_births, aes(x = day, y = total_births_daily, color = wday)) +
  geom_point()
```
...
```{r}
Birthdays |>
  mutate(month_name = month(make_date(year, month, 1), label = TRUE)) |>
  group_by(year, month_name) |>
  summarize(total_monthly = sum(births)) |>
  ggplot(aes(y = total_monthly, x = month_name)) +
  geom_point()
```
...
Birthdays |>
  filter(year == "1988") |>
  mutate(day_of_year = yday(make_date(year, month, day))) |>
  mutate(week = ceiling(day_of_year / 7)) |>
  filter(week <= 52) |>
  group_by(state, week) |>
  summarize(total_weekly_births = sum(births, na.rm = TRUE)) |>
  ggplot(aes(x = week, y = total_weekly_births, color = state, group = state)) +
  geom_line(alpha = 0.7)
```
...
Birthdays |> 
  group_by(state) |>
  filter(date >= "1979-10-09" & date <= "1979-10-12") |>
