...
Our goal is to *combine* or *join* these datasets into one. 
...
-   What did this do? What are the roles of `students_1` (the *left* table) and `enrollments_1` (the *right* table)?
  - This linked course info with students and removed any missing data. 
-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
...
-   What did this do? What are the roles of `students_1` (the *left* table) and `enrollments_1` (the *right* table)?
  - It retains all info from both datasets. 
-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
...
-   What did this do? What info would it give us?
  - It tells us which students have info in enrollment. 
-   How does `semi_join()` differ from `inner_join()`?
  - Semi join only keeps info from the left dataset. It removes any data points that weren't in both datasets, however. 
-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
...
-   What did this do? What info would it give us?
  - It tells us which observation is not included in the second dataset. 
-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
...
grades |> 
  left_join(gpa_conversion) |> 
  mutate(below = (gp < 3.3)) |> 
  summarize(mean(below, na.rm = TRUE))
```
